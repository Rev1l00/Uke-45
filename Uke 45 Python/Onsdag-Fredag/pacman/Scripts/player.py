...
player_speed = 3
def powered_up(speed):
    while player_speed == 3:
        from speed_increase import power_up
        if power_up == True:
            speed += 10
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self,x,y,filename):
        ...
        self.image = pygame.image.load("Onsdag-Fredag\pacman\img\Pacman\player.png").convert()
        self.image.set_colorkey(BLACK)
        self.rect = self.image.get_rect()
        self.rect.topleft = (x,y)
        # Load animation
        img = pygame.image.load("Onsdag-Fredag\pacman\img\Pacman\walk.png").convert()
        ...
        img = pygame.image.load("Onsdag-Fredag\pacman\img\Pacman\explosion.png").convert()
        self.explosion_animation = Animation(img,30,30)
        # Save the player image
        self.player_image = pygame.image.load("Onsdag-Fredag\pacman\img\Pacman\player.png").convert()
    ...
    def move_right(self):
        self.change_x = int(powered_up(player_speed))

    def move_left(self):
        self.change_x = -1 * int(powered_up(player_speed))

    def move_up(self):
        self.change_y = -1 * int(powered_up(player_speed))

    def move_down(self):
        self.change_y = int(powered_up(player_speed))
